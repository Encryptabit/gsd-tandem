...
must_haves:
  truths:
    ...
    - "create_review validates diffs on submission/revision and rejects invalid diffs before persistence"
    - "Reviewer claiming a review with a diff re-validates with git apply --check inside the write_lock"
    ...
    - "Notes are required (non-whitespace) for changes_requested and comment verdicts but optional for approve"
  ...
  key_links:
    ...
    - from: "tools.py create_review"
      to: "validate_diff"
      via: "submission-time validation before INSERT/UPDATE when diff exists"
...
Purpose: With the schema, models, and diff_utils in place from Plan 01, this plan wires everything together in the tool layer -- the actual MCP interface that proposer and reviewer agents interact with. After this plan, the broker supports the full proposal lifecycle: submit with diff, validate on submit, re-validate on claim, issue typed verdicts, revise proposals, and retrieve proposal content.

Output: Extended tools.py with updated create_review (description + diff + revision + submit-time validation), claim_review (claim-time re-validation + inline content), submit_verdict (comment type + strict notes enforcement), new get_proposal tool, plus comprehensive tests in test_proposals.py and extended test_tools.py.
</objective>
...
<task type="auto">
  ...
  <action>
    ...
    **1. Extend create_review with proposal content + revision flow:**
    ...
    - When `diff` is provided, call `await validate_diff(diff, cwd=app.repo_root)` before any INSERT/UPDATE. If invalid, return an error dict with `validation_error` and do not persist changes.
    ...
    **2. Extend claim_review with diff re-validation and inline content:**
    ...
    - After validating the transition to CLAIMED but BEFORE performing the UPDATE, check if the review has a diff:
      - If `diff_text = row["diff"]` is truthy, call `await validate_diff(diff_text, cwd=app.repo_root)` INSIDE the write_lock (defense-in-depth re-validation at claim time, and prevents wasted subprocess calls on concurrent claims).
    ...
    **3. Extend submit_verdict with comment type and notes enforcement:**
    ...
    - Enforce notes requirement: if verdict is "changes_requested" or "comment" and reason is None, empty, or whitespace-only, return error dict: `{"error": "Notes (reason) required for 'changes_requested' verdict."}` or `{"error": "Notes (reason) required for 'comment' verdict."}`.
  ...
  <verify>
    Run `uv run python -c "from gsd_review_broker.tools import get_proposal; print('get_proposal registered')"` from tools/gsd-review-broker/ to confirm the new tool imports. Run `uv run --extra dev pytest tests/test_diff_utils.py -v` to ensure Plan 01 tests still pass (no import breakage). Run `uv run --extra dev ruff check src/` to check for lint issues.
  </verify>
  <done>
...
<task type="auto">
  ...
  <action>
    ...
    **2. Extend test_tools.py** with new verdict tests:
    - **TestSubmitVerdict**: Add tests for:
      ...
      - `test_submit_verdict_comment_whitespace_only_notes_rejected`: Create review, claim, submit "comment" with whitespace-only reason. Verify error dict returned.
      ...
      - `test_submit_verdict_changes_requested_whitespace_notes_rejected`: Create review, claim, submit "changes_requested" with whitespace-only reason. Verify error dict returned.
    ...
    **3. Create test_proposals.py** with comprehensive proposal lifecycle tests:
    ...
    - **TestCreateReviewWithProposal:**
      ...
      - `test_create_review_rejects_invalid_diff_on_submission`: Mock validate_diff to fail. Verify create_review returns error + validation_error and no row is inserted.
    ...
    - **TestFullProposalLifecycle:**
      ...
      - `test_proposal_lifecycle_submit_reject_revise_approve`: Create review with description + diff. Claim (mock validate_diff if needed), changes_requested with notes, revise with new content, re-claim, approve, close. Verify revision clears old state and full lifecycle completes.
  ...
  <verify>
    Run `uv run --extra dev pytest -v` from tools/gsd-review-broker/. All tests pass: existing 44 tests + new verdict tests in test_tools.py + new proposal tests in test_proposals.py. Run `uv run --extra dev pytest --tb=short` for a clean overview. Verify test count is at least 60 (44 existing + ~4 verdict + ~14 proposal).
  </verify>
  <done>
...
<verification>
1. `uv run --extra dev pytest -v` from tools/gsd-review-broker/ passes all tests (>= 60 total)
...
4. create_review calls validate_diff when diff present and rejects invalid diffs before persistence
5. claim_review re-validates via validate_diff when diff present and auto-rejects on failure
6. submit_verdict supports "comment" verdict type without state transition
7. submit_verdict enforces non-whitespace notes for changes_requested and comment
8. get_proposal returns full proposal content including diff
9. Revision flow: changes_requested -> create_review(review_id=X) -> pending with cleared reviewer state
</verification>

<success_criteria>
- All 7 MCP tools work correctly per Phase 2 requirements
- Proposal creation stores description, diff, and affected_files
- Diff validation runs on submission and re-runs on claim; bad diffs are rejected with clear error detail
- Comment verdict records feedback without state transition
- Notes enforcement: non-whitespace notes required for changes_requested/comment, optional for approve
