...
<task type="auto">
  ...
  <action>
    ...
    2. **Update db.py schema with V2 migrations**:
       ...
       - Update `ensure_schema` to iterate `SCHEMA_MIGRATIONS` after executescript, ignoring only duplicate-column migration errors while surfacing unrelated SQL failures.
