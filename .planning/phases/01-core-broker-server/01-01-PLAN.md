...
<task type="auto">
  ...
  <verify>
    ...
    - Start server: `uv run gsd-review-broker` -- confirm it starts and binds to 127.0.0.1:8321 (check output for "Uvicorn running on http://127.0.0.1:8321"), then Ctrl+C to stop
    - Verify SQLite database was created: confirm `.planning/codex_review_broker.sqlite3` exists after server start
...
2. `uv run gsd-review-broker` starts server on 127.0.0.1:8321
3. `.planning/codex_review_broker.sqlite3` is created with `reviews` table
4. Server shutdown is clean (no locked file warnings)
