...
must_haves:
  truths:
    ...
    - "SpawnConfig validates that workspace_path exists on disk (on native platforms)"
...
<task type="auto">
  ...
  **config_schema.py** -- Create new module with:
  ...
  2. `SpawnConfig(BaseModel)` Pydantic model with:
     ...
     **workspace_path existence validation (per user decision on "path existence checks"):**
     Add a `@field_validator("workspace_path")` that validates the path exists on disk. Use `pathlib.Path(v).exists()` for the check. However, on Windows (`os.name == "nt"`), WSL paths like `/mnt/c/Projects/...` cannot be validated from the Windows side since they are Linux filesystem paths. So the validator should:
     - If `os.name != "nt"` (native Linux/macOS): check `Path(v).exists()`, raise ValueError if not found.
     - If `os.name == "nt"` (Windows): skip the existence check (WSL paths are not resolvable from Windows Python). The path will be validated implicitly when the subprocess runs.
     This fulfills the locked decision for path existence checks while remaining cross-platform safe.
  ...
  **test_config_schema.py** -- Create test file with:
  ...
  - test_workspace_path_nonexistent_rejected: Mock `os.name` to return a non-"nt" value (e.g., "posix") to simulate a native Linux/macOS platform, then construct SpawnConfig with a workspace_path that does not exist (e.g., "/tmp/nonexistent_workspace_path_xyz123"). Verify that Pydantic raises ValidationError with a message indicating the path does not exist. This validates the @field_validator("workspace_path") enforces path existence on native platforms per the locked decision for "path existence checks".
  - test_workspace_path_exists_accepted: On native platform mock, create a temp directory, construct SpawnConfig with that temp dir as workspace_path, verify it succeeds.
  - test_workspace_path_skips_check_on_windows: Mock `os.name` to return "nt", construct SpawnConfig with a non-existent workspace_path (e.g., "/mnt/c/nonexistent"). Verify it does NOT raise -- the check is skipped on Windows since WSL paths cannot be resolved.
  ...
  **test_db_schema.py** -- Add tests that the reviewers table and new review columns exist after ensure_schema:
  ...
  <done>
  SpawnConfig validates all spawn parameters with allowlists and range checks. SpawnConfig validates workspace_path exists on native platforms (skips on Windows for WSL path compatibility). load_spawn_config returns None when reviewer_pool key is missing (pool disabled). Reviewer prompt template exists. Schema tests confirm reviewers table and fence token columns. Config tests confirm validation catches invalid models, metacharacters, nonexistent workspace paths, and missing-key case.
  </done>
</task>
...
- SpawnConfig rejects nonexistent workspace_path on native platforms (not Windows)
...
<success_criteria>
All schema, model, config, and state machine foundations are in place for the ReviewerPool (plan 02) and fenced reclaim (plan 03) to build on. All tests pass including new schema and config validation tests. load_spawn_config behavior is canonical: missing key returns None (pool disabled). SpawnConfig validates workspace_path existence on native platforms per locked decision.
</success_criteria>
