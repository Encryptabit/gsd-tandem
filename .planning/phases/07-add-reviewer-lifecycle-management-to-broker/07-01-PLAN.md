...
must_haves:
  ...
  artifacts:
    - path: "tools/gsd-review-broker/src/gsd_review_broker/config_schema.py"
      ...
      exports: ["SpawnConfig", "ALLOWED_MODELS"]
...
<task type="auto">
  ...
  **config_schema.py** -- Create new module with:
  ...
  3. `load_spawn_config(config_path: str | Path) -> SpawnConfig` function that reads a JSON config file, extracts a `"reviewer_pool"` key, and constructs SpawnConfig. If the key is missing, returns SpawnConfig with only workspace_path inferred from repo_root (raise ValueError if neither available). If the file doesn't exist, raise FileNotFoundError.
  ...
  **test_config_schema.py** -- Create test file with:
  ...
  - test_load_spawn_config_from_file: write a temp JSON file with reviewer_pool key, load it, verify
  ...
  **test_db_schema.py** -- Add tests that the reviewers table and new review columns exist after ensure_schema:
  ...
  <done>
  SpawnConfig validates all spawn parameters with allowlists and range checks. Reviewer prompt template exists. Schema tests confirm reviewers table and fence token columns. Config tests confirm validation catches invalid models and metacharacters.
  </done>
</task>
...
<success_criteria>
All schema, model, config, and state machine foundations are in place for the ReviewerPool (plan 02) and fenced reclaim (plan 03) to build on. All tests pass including new schema and config validation tests.
</success_criteria>
