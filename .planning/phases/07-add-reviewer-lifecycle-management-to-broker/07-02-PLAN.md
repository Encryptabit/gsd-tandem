...
must_haves:
  truths:
    ...
    - "stdout/stderr use DEVNULL to prevent pipe buffer deadlock on long-running reviewers"
    - "Orphan process is terminated if DB persistence fails after spawn"
    - "load_prompt_template substitutes all template placeholders with no unresolved braces remaining"
  artifacts:
    ...
    - path: "tools/gsd-review-broker/src/gsd_review_broker/platform_spawn.py"
      ...
      exports: ["detect_platform", "build_codex_argv", "load_prompt_template"]
      min_lines: 30
...
<task type="auto">
  ...
  **platform_spawn.py** -- Create new module with:
  ...
  3. `load_prompt_template(template_path: str | Path, reviewer_id: str) -> str`: Load the reviewer_prompt.md template and substitute ALL template placeholders, returning the prompt string.
     - Substitute `{reviewer_id}` with the provided reviewer_id parameter.
     - Substitute `{claim_generation_note}` with a standard instructional string, e.g.: `"IMPORTANT: When submitting a verdict, always pass the claim_generation value you received from the claim_review response. This prevents stale verdicts if your claim was reclaimed due to timeout."`.
     - Use `str.replace()` (NOT `str.format()`) for substitution to avoid KeyError on any brace-like content in the template. Replace `{reviewer_id}` first, then `{claim_generation_note}`.
     - After substitution, verify no unresolved `{...}` template placeholders remain. Use a simple regex check (e.g., `re.search(r'\{[a-z_]+\}', result)`) and raise ValueError if any unresolved placeholders are found. This catches any future template variables that are added to the template but not handled in this function.
  ...
  **test_platform_spawn.py** -- Create test file with:
  ...
  - test_load_prompt_template: write a temp file with `{reviewer_id}` and `{claim_generation_note}` placeholders, load it, verify both are substituted (reviewer_id replaced with actual ID, claim_generation_note replaced with instructional text)
  - test_load_prompt_template_no_unresolved_placeholders: write a temp file with `{reviewer_id}`, `{claim_generation_note}`, and an extra unknown placeholder like `{unknown_var}`. Call load_prompt_template. Verify it raises ValueError because `{unknown_var}` remains unresolved after substitution. This validates the safety net that catches template/code mismatches.
  - test_load_prompt_template_all_known_placeholders_resolved: write a temp file containing ONLY `{reviewer_id}` and `{claim_generation_note}`. Call load_prompt_template. Verify the result contains NO brace-enclosed placeholders (all resolved).
  ...
  <done>
  Platform detection and shell-free argv builder work correctly on both Windows (WSL) and native platforms. Template loading substitutes ALL placeholders (reviewer_id and claim_generation_note) using str.replace(). Unresolved placeholder detection raises ValueError to catch template/code drift. Tests confirm no shell metacharacter expansion and full placeholder substitution.
  </done>
</task>
...
<task type="auto">
  ...
  **Methods:**
  ...
  1. `async def spawn_reviewer(self, db: aiosqlite.Connection, write_lock: asyncio.Lock) -> dict`:
     ...
     - Spawn subprocess: `asyncio.create_subprocess_exec(*argv, stdin=PIPE, stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL)`.
       **CRITICAL: Use DEVNULL for stdout and stderr, NOT PIPE.** Long-running reviewer subprocesses will block when OS pipe buffers fill (~64KB) if stdout/stderr are PIPE with no reader. We do not need to capture reviewer output -- the reviewer communicates via MCP tools, not stdout.
     - Write prompt to stdin, close stdin (do NOT use communicate() since process is long-running -- stdin.write() + stdin.close() is correct).
     - Store process in `_processes[reviewer_id]`.
     - Update _last_spawn_time.
     - **DB persistence with orphan protection:** Wrap the DB write in try/except. Inside write_lock with BEGIN IMMEDIATE, insert row into `reviewers` table and record `reviewer_spawned` audit event with metadata {reviewer_id, display_name, pid}. If the DB write raises ANY exception: immediately call process.terminate() on the spawned process to prevent an orphan (untracked process with no DB record), remove reviewer_id from _processes, re-raise or return error dict explaining the failure. This prevents state divergence where a process is running but has no tracking record.
  ...
  **test_pool.py** -- Create test file. Since subprocess spawning needs mocking (we don't have codex in test env):
  ...
  - test_spawn_uses_devnull_for_stdout_stderr: verify the mock was called with stdout=asyncio.subprocess.DEVNULL and stderr=asyncio.subprocess.DEVNULL (not PIPE). This prevents pipe buffer deadlock on long-running reviewer subprocesses.
  - test_spawn_db_failure_terminates_orphan: Mock the DB write (inside write_lock) to raise an exception. Verify that: (a) the spawned process's terminate() method was called, (b) reviewer_id was removed from _processes, (c) an error is returned (not a success dict). This validates the orphan protection mechanism.
  ...
  <verify>
  Run `cd C:/Projects/gsd-tandem/tools/gsd-review-broker && uv run pytest tests/test_pool.py tests/test_platform_spawn.py -v` -- all tests pass. Verify pool.py has no shell=True usage. Verify write_lock is NOT held during subprocess spawn. Verify stdout/stderr use DEVNULL not PIPE. Verify spawn_reviewer has try/except around DB persistence with process termination in the except block.
  </verify>
  <done>
  ReviewerPool spawns, drains, and terminates Codex reviewer subprocesses. stdout/stderr use DEVNULL to prevent pipe buffer deadlock. Orphan process protection terminates subprocess on DB write failure. All operations are shell-free, use asyncio.create_subprocess_exec, and record audit events. Per-reviewer stats tracked. Tests verify lifecycle with mocked subprocesses including DEVNULL and orphan protection.
  </done>
</task>
...
- pool.py uses `stdout=asyncio.subprocess.DEVNULL, stderr=asyncio.subprocess.DEVNULL` (NOT PIPE)
- pool.py spawn_reviewer has try/except around DB persistence that terminates orphan process on failure
- platform_spawn.py returns list[str] argv with no command-string assembly
- platform_spawn.py load_prompt_template substitutes BOTH {reviewer_id} and {claim_generation_note}
- platform_spawn.py load_prompt_template raises ValueError on unresolved placeholders
...
<success_criteria>
ReviewerPool can spawn, drain, and terminate reviewer subprocesses with full audit trail. stdout/stderr are DEVNULL (no pipe deadlock). DB write failures terminate orphan processes. Platform-aware argv builder produces correct commands for Windows (WSL) and native platforms. No subprocess uses shell=True. load_prompt_template substitutes all template placeholders and detects unresolved ones.
</success_criteria>
