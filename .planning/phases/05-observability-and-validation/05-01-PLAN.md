...
must_haves:
  truths:
    ...
    - "Stats tool returns total reviews, approval/rejection rates, reviews by category, average time-to-verdict, average review duration, and average time in each state"
...
<task type="auto">
  ...
  <action>
    ...
    **Tool 1: get_activity_feed(status: str | None = None, category: str | None = None, ctx: Context = None) -> dict**
    ...
    - ORDER BY r.updated_at DESC, r.id DESC (stable ordering when timestamps tie)
    ...
    **Tool 3: get_review_stats(ctx: Context = None) -> dict**
    ...
    - Query 6: Average time in each state (pending, claimed, approved, changes_requested) in seconds. Use consecutive audit_events for the same review (`LEAD(created_at)` over `PARTITION BY review_id ORDER BY id`) and aggregate durations by `new_status`; for rows with no next event, treat duration as null and exclude from average.
    - Round numeric values to 1 decimal place where not null.
    - Return {"total_reviews": N, "by_status": {...}, "by_category": {...}, "approval_rate_pct": N|null, "avg_time_to_verdict_seconds": N|null, "avg_review_duration_seconds": N|null, "avg_time_in_state_seconds": {"pending": N|null, "claimed": N|null, "approved": N|null, "changes_requested": N|null}}
...
- get_review_stats returns counts, rates, timing metrics, and average time-in-state metrics (null-safe for empty databases)
