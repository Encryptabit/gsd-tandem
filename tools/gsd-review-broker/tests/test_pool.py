...
def test_resolve_workspace_prefers_repo_root_over_stale_base(tmp_path: Path) -> None:
    projects_root = tmp_path / "Projects"
    projects_root.mkdir()
    target = projects_root / "Code2Obsidian"
    target.mkdir()

    stale_base = tmp_path / "mnt" / "c" / "Projects" / "ams"
    stale_base.mkdir(parents=True)

    cfg = SpawnConfig(
        workspace_path=str(stale_base),
        prompt_template_path="reviewer_prompt.md",
        spawn_cooldown_seconds=1.0,
        max_pool_size=3,
        model="o4-mini",
...
async def test_spawn_reviewer_applies_project_local_overrides(
    tmp_path: Path, db: aiosqlite.Connection, monkeypatch: pytest.MonkeyPatch
) -> None:
    projects_root = tmp_path / "Projects"
    projects_root.mkdir()
    ...
    (target / ".planning" / "config.json").write_text(
        json.dumps(
            {
                "reviewer_pool": {
                    "model": "gpt-5-codex",
                    "reasoning_effort": "medium",
                    "workspace_path": "/mnt/c/Projects/ams",
                }
            }
        ),
        encoding="utf-8",
    ...
    pool = ReviewerPool(session_token="abcd1234", config=cfg, repo_root=str(projects_root))
    ...
    captured_configs: list[SpawnConfig] = []
    monkeypatch.setattr(
        "gsd_review_broker.pool.build_codex_argv",
        lambda cfg: captured_configs.append(cfg) or ["codex", "-"],
    )
    monkeypatch.setattr(
        "gsd_review_broker.pool.load_prompt_template",
        lambda _path, reviewer_id: f"prompt:{reviewer_id}",
    ...
    assert used.model == "gpt-5-codex"
    assert used.reasoning_effort == "medium"
    assert used.workspace_path == str(target)
